...
jpeg_decoder::jpeg_decoder(std::string filename) : zigZagStart(0), zigZagEnd(63), losslessFormat(false) {
    ...
    // TODO: check this memory issue
    // initialize the raw image pointers to null
    for(int i = 0; i < ETF_FORMAT_MAX_COMPONENTS; ++i)
    {
        rawImagePointers[i] = NULL;
    }
    ...
    // init the coefficients buffer into null:
     m_YPicture_buffer_enc = m_CbPicture_buffer_enc = m_CrPicture_buffer_enc = NULL;
...
jpeg_decoder::~jpeg_decoder() {
    ...
    // TODO: you need to check this
    deleteRawPictureBufferPointersEnc();
...
uint_16 jpeg_decoder::readBits(int len) {
    ...
    if (len > queueLen) {
        do {
            // Read a byte in, shift it up to join the queue from the left
            readByte = fgetc(fp);
...
uint_16 jpeg_decoder::readFrameHeader(uint_16 headerLengthFromBitStream)
{
	...
	upscale_width =  ceil(1.0* jpegImageWidth / SUBSAMPLING_FACTOR_DEC) * SUBSAMPLING_FACTOR_DEC;
	upscale_height = ceil(1.0* jpegImageHeight / SUBSAMPLING_FACTOR_DEC) * SUBSAMPLING_FACTOR_DEC;
...
uint_16 jpeg_decoder::readImageData() {
    ...
    if (losslessFormat) {
        // TODO: losless format is not really supported here, but I added the code for it
    ...
    for(int i = 0; i < components.size(); ++i)
    {
        rawImagePointers[i] = new unsigned char[(jpegImageWidth*jpegImageHeight)/(components.at(i).HScale * components.at(i).VScale)];
    }
    
    
    lineBytes = static_cast <int>(jpegImageWidth * components.size());
    if (precision == 12) {
        lineBytes *= 2;
    }
    ...
    {
#if DEBUGLEVEL>1
        cout << "--- Huffman read block " << blkNo << " component "<< currentComponent  << endl;
#endif
        
        // 64 DC and AC coefficients are stored in row
        readHuffmanBlock(row, currentComponent);
        
        // TODO: Support lossless format
        if (losslessFormat) {
            //            addLossless(row, currentComponent);
            cout << "Lossless format is not supported yet! " << endl;
            break;
            
        }
        else {
            // DC predictor (update the previous DC coeffient)
            ...
            // Adding data to ImageState old
            if (hasSubSampling) {
                addBlockSubsamplingOld(row3, currentComponent);
            }
            else {
                addBlockOld(row3, currentComponent);
            }
            
        } // end else lossy format
    ...
    if (losslessFormat) {
        ...
        // TODO: losless is not supported; putting the code for it
        // Predictor requires to hold last line in memory
        for (uint i=0; i<components.size(); ++i)
    ...
    {
        cout << "XXXX Delete rawImagePointers " << endl;
        //        for(int i = 0; i < components.size(); ++i)
        //        {
        //            delete [] rawImagePointers[i];
        //        }
    }
...
#endif
//  TODO: remove this
	//for (uint y = 0; y < upscale_height; y += 8) {
	//	for (uint x = 0; x < upscale_width; x += 8) {
	//		//cout << "X: " << x << ", Y: " << y << endl;
	//		ofstream myfile;
	//		std::string path_to_files = "C:/Users/y77jiang/OneDrive - University of Waterloo/5e. TCM-Inception C++/JPEG/JPEG/";
	//		std::string output_csv_name = path_to_files + "Lena_Y_dec.csv";
	...
	//				int real_x = i + x;
	//				int real_y = y + j;
	//				int val = tCoeff_Y[real_y][real_x];
	//				myfile << val << ",";
	//			}
	//			
	//		myfile << "\n";
...
//    // TODO: remove FANKOOSH
    ...
    if(currentComponent == COMPONENT_Cb) {
        ...
        ofstream myfile;
        std::string path_to_files = "C:/Users/y77jiang/OneDrive - University of Waterloo/5e. TCM-Inception C++/JPEG/JPEG/";
        std::string output_csv_name = path_to_files + "Nature_Cb_dec.csv";
        myfile.open (output_csv_name, std::ofstream::out | std::ofstream::app);
...
float jpeg_decoder::C_dct(int u) {
...
int jpeg_decoder::func_dct(int x, int y, const int block[8][8]) {
...
void jpeg_decoder::perform_fdct(int outBlock [8][8], int inBlock[8][8]) {
...
// tCoeff: Transformed DCT coefficients coming from bitstream
// out_block: output of the current DCT operation
// in_block: The output of the IDCT operation from bitstream (YCbCr)
void jpeg_decoder::Fankoosh(int tCoeff [8][8], int out_block8x8[8][8], int in_block8x8[8][8], int currentComponent) {
    
    
    printf("# output of the IDCT operation from bitstream (YCbCr) 8x8 Block#\n");
    for(int i = 0; i < 8; ++i){
        for(int j = 0; j < 8; ++j){
            cout << std::dec <<  in_block8x8[i][j] << " ";
        }
        
        cout << "\n";
    }
    
    // start FDCT for each 8x8 block
    perform_fdct(out_block8x8, in_block8x8);
    
    printf("# [After DCT] Correct Transformed Coefficients 8x8 Block#\n");
    for(int i = 0; i < 8; ++i){
        for(int j = 0; j < 8; ++j){
            cout << std::dec <<  tCoeff[i][j] << " ";
        }
        
        cout << "\n";
    }
    
    printf("# [By doing DCT] My Transformed Coeff 8x8 Block#\n");
    for(int i = 0; i < 8; ++i){
        for(int j = 0; j < 8; ++j){
            cout << std::dec <<  out_block8x8[i][j] << " ";
        }
    ...
    int temp [8][8] = {0};
    perform_idct(temp, out_block8x8);
    
    printf("# [By doing IDCT here] My YCbCr 8x8 Block#\n");
    for(int i = 0; i < 8; ++i){
        for(int j = 0; j < 8; ++j){
            // transpose
            cout << std::dec <<  temp[i][j] << " ";
        }
...
// TODO: take into consideration that imgw and imgh may not be multiples of 8x8
void jpeg_decoder::ycrcb_to_rgb24_block8x8(int w, int h, int imgx, int imgy, int imgw, int imgh) {
    
    const uint_8 *Y, *Cb, *Cr;
    uint_8 *pix;
    ...
    // TODO: nComponents may not be 3; you need to handle that
    ...
    for (int y = 0; y < (8 * h - olh); ++y)
    {
        ...
        {
            int poff = x * nRGBComponents + jpegImageWidth * nRGBComponents * y;
            ...
            // TODO: Support for grayscale components and with various sample precision
...
void jpeg_decoder::ycrcb_to_rgb24_image() {
    ...
    for (int y = 0; y < jpegImageHeight; ++y) {
        for (int x = 0; x < jpegImageWidth; ++x) {
...
void jpeg_decoder::readHuffmanBlock(int *dataBlock, int currentComponent) {
    
    // Data bytes must be preserved for next block, so they are marked static
    static uint_32 data = 0;
    static uint_32 currentDataLength = 0;
    
    // Debugging
    static uint_32 byteno = 0;
    ...
    do
    {
        ...
        // Every one of 256 elements of the current Huffman table
        // potentially has value, so we must go through all of them
        for (int i = 0; i < 256; ++i) {
            ...
            // If current data length is greater or equal than n,
            // compare first n bits (n - length of current Huffman code)
            ...
            if (currentDataLength >= n && htable->codes[i] == data >> (currentDataLength-n)) {
                ...
                if (ACDC == DC) {
                    ...
                    while (currentDataLength < bitLength) {
                        if (!readMoreData(data, currentDataLength)) {
                            endOfFile = true;
#if DEBUGLEVEL>0
                            cout << "End of file encountered inside a Huffman code!" << endl;
#endif
                            break;
                        }
                        byteno++;
                        ...
                        cout << "Byte(+) " << byteno << " CDL " << currentDataLength <<" value " << hex << data << dec <<endl;
#endif
                    } // end while (currentDataLength < bitLength)
                    ...
                    if (currentDataLength < 3) // If currentData length is < than 3, we need to read new byte, so leave this for loop
                        break;
                    // Hossam: XXXX Maybe a source of error
                ...
                // Reading of AC coefficients
                    ...
                    /* Every AC component is composite of 4 bits (RRRRSSSS). R bits tells us relative position of non zero element from the previous non zero element (number of zeros between two non zero elements) SSSS bits tels us magnitude range of AC element
                     Two special values:
                    ...
                    if (ACElement == 0x00) {
                        ...
                        for (int k = 0;k < 16; ++k) {
                            ...
                            if (m >= ACcount+1) {
#if DEBUGLEVEL>0
                                cout << "Huffman error: 16 AC zeros requested, but only "<< k <<" left in block!" << endl;
#endif
                                return;
                            } // end if (m >= ACcount+1)
                        } // end for (int k = 0;k < 16; ++k) {
                    } // end else if (ACElement == 0xF0)
                    
                    // zero AC and normal case:
                    else {
                        
                        /* If AC element is 0xAB for example, then we have to separate it in two nibbles
                         * First nible is RRRR bits, second are SSSS bits
                         * RRRR bits told us how many zero elements are before this element
                         * SSSS bits told us how many binary digits our AC element has (if 1001
                         * then we have to read next 9 elements from file)
                         */
                        
                        // Let's separate byte to two nibles
                        uint_8 Rbits = ACElement >> 4;
                        uint_8 Sbits = ACElement & 0x0F;
                        
                        // Before our element there is Rbits zero elements
                        for (int k = 0; k < Rbits; ++k) {
                            if (m >= ACcount) {
#if DEBUGLEVEL>0
                                cout << "Huffman error: "<< Rbits <<" preceeding AC zeros requested, but only " << k <<" left in block!" << endl;
#endif
                                // in case of error, doing the other stuff will just do more errors so return here
                                return;
                            } // end if (m >= ACcount)
...
#endif
                        } // end while (currentDataLength < Sbits)
                    ...
                    } // end else zero ACs and normal case
...
#if DEBUGLEVEL > 4
                        cout << "If currentData length is < 3, we need to read new byte, so leave this for loop" << endl;
#endif
                        break;
                    }
                    i = -1; // currentDataLength is not zero, set i=0 to start from first element of array
                    
                } // end else reading AC coefficients
                
            } // end if (currentDataLength >= n && htable->codes[i] == data >> (currentDataLength-n))
            
        } // end for loop on the lengths
        
#if DEBUGLEVEL > 4
        cout << "Looping while(!readMoreData(data, currentDataLength));" << endl;
#endif
        
    }while(!readMoreData(data, currentDataLength));
    
    
#if DEBUGLEVEL > 4
    cout << "End of file from readHuffmanBlock " << endl;
#endif
    
    
    endOfFile = true; // We reached an end
    
} // end readHuffmanBlock

bool jpeg_decoder::readMoreData(unsigned int &data, unsigned int &currentDataLength) {
    
    uint_8 binaryData;
    
#if DEBUGLEVEL > 2
    cout << "Hello from readMoreData" << endl;
#endif
    // Detect errors
    ...
    {
#if DEBUGLEVEL > 2
        cout << "Binary Data " << std::hex << binaryData << " readMoreData terminated the blocks " << endl;
#endif
    ...
    if (binaryData == 0xFF) {
...
#if DEBUGLEVEL > 1
            cout << "(2) Binary Data " << std::hex << binaryData << " readMoreData terminated the blocks " << endl;
#endif
            return false; // End of file
        }
        ...
        } // end if (binaryData >= 0xd0 && binaryData <= 0xd7)
        // If after FF byte comes 0x00 byte, we ignore it, 0xFF is part of data (byte stuffing)
            ...
            cout << "Stuffing" <<endl;
#endif
        } // end else if (binaryData != 0)
    } // end if (binaryData == 0xFF)
    
    else {
        
#if DEBUGLEVEL > 2
        cout << "Binary Data " << std::hex << binaryData << " readMoreData worked fine " << endl;
#endif
        data = (data << 8) + binaryData;
        currentDataLength += 8;
    }
 ...
 */
// TODO: adjust this method to work in the case when no subsampling
// TODO: copy stuff from addBlockSubsampling for this method
void jpeg_decoder::addBlock(int dataBlock[8][8], int comp /* Current component */ ) {
    ...
    for (y = 0; y < 8; ++y) {
        int x;
        for (x = 0; x < 8 && currentX[comp] < width; ++x) { // increment currentX here
            
            switch(jpegImageSamplePrecision) {
                case 8:
                    break;
                case 12:
                    cout << "-|- ##ERROR## JPEG Sample precision is 12 bits; will assume it is 8 bits. Most probably will fail! " << endl;
                    break;
                default:
                    cout << "-|- ##ERROR## No JPEG Sample precision - I will assume that my precision is 8 bits. Most probably will fail! " << endl;
                    break;
                    
            } // end switch
            
            // TODO: check if it is x and y or y and x
            
            // Read picture_x and update current_x
            int picture_x = currentX[comp]++; int picture_y = currentY[comp];
            //            int value = dataBlock[y][x];
            int value = dataBlock[x][y];
            m_YPicture_buffer[picture_x][picture_y] = value;
            
            
        } // end inner
        
        // reset current X to its initial point
        currentX[comp] -= x;
        
        // Increment currentY to next line, but break if you're above image height
    ...
    if (currentX[comp] > width) {
        ...
        // reset currentX for next block to zero
        currentX[comp] = 0;
...
// TODO: this method saves only the Y component -- future support for CbCr should be easy
// Just switch on the Y;Cb;Cr buffer while saving
                ...
                for (int i = 0; i < HScale; ++i) {
                    ...
                    switch(comp) {
                        case COMPONENT_Y:
                            ...
                            // TODO: check this
                            // Add the raw value
                            m_YPicture_buffer_enc[picture_y][realx] = value;
                        ...
                        case COMPONENT_Cb:
                            ...
                            // TODO: check this
                            // Add the raw value
                            m_CbPicture_buffer_enc[picture_y][realx] = value;
...
//                            if(debug ++ < 257)
//                            {
//                                cout << "X: " << realx << ", Y: " << picture_y << ", val: " <<  m_CbPicture_buffer_enc[picture_y][realx] << endl;
//                            }
                            break;
                        case COMPONENT_Cr:
                            ...
                            // TODO: check this
                            // Add the raw value
                            m_CrPicture_buffer_enc[picture_y][realx] = value;
...
/* This method will be called repeatedly to place 8x8 blocks on the picture, from
 left to right and then onto next line.
 */
void jpeg_decoder::addBlockOld(int dataBlock[8][8], int comp /* Current component */ ) {
    
    // Avoid function calls... these functions are inline but the library is external...
    int width = jpegImageWidth, height = jpegImageHeight;
    // Hossam: added a casting here
    int pixelJump = static_cast<int>(components.size());
    if (precision == 12) {
        pixelJump *= 2;
    }
    
    // raster block sanning:
    int y;
    for (y = 0; y < 8; ++y) {
        int x;
        for (x = 0; x < 8 && currentX[comp] < width; x++, currentX[comp]++) {
            ...
            // TODO: Support for CMYK is not done here
            //            if (imagePtr->format().getColorModel() == Format::CMYK)
            //                value=maxSample-value;
            
            // Store the Y - jump 3 pixls and then put another Y...etc (pattern is YCbCr YCbCr..etc)
            if (jpegImageSamplePrecision == 8) {
        ...
        // Move currentX and rawImagePointer to where they were before x loop (reset to initial position)
        currentX[comp] -= x;
...
/* This method is much more complex, having to deal with subsampling.
 Also somewhat slower due to all the counters. */
void jpeg_decoder::addBlockSubsamplingOld(int dataBlock[8][8], int comp /* Current component */ ) {
    ...
    // Hossam: added a casting here
    int pixelJump = static_cast<int>(components.size());
    if (jpegImageSamplePrecision == 12) pixelJump *= 2;
    ...
    for (uint y = 0; y < 8; ++y) {
        ...
        for (int vfy = 0; vfy < VScale; ++vfy) {
            ...
            // XXXX Hossam: this one can be removed
            ...
            for (uint x = 0; x < 8; ++x) {
                ...
                int realx = currentX[comp] + x * HScale;
                for (int i = 0; i < HScale; ++i) {
                    // Hossam: XXXX remove this
                    //                    p.setComponent(comp, value);
                    //                    p.next();
                    
                    // Hossam: XXXX add this
                    if (jpegImageSamplePrecision == 8) {
                        *rawImagePointers[comp] = value;
                        rawImagePointers[comp] += pixelJump;
...
int jpeg_decoder::display_jpg_bmp(const std::string output_bmp_filename) const {
    ...
    oss.clear(); oss.str("");
    //    oss << name_file_only << "_" << jpegImageWidth << "x" << jpegImageHeight << ".bmp";
...
void jpeg_decoder::initPositionsBuffersForPictureBuffer() {
    ...
    int comp_size = ceil((comp_width * comp_height)/
                        ((double) components.at(COMPONENT_Y).HScale* (double) components.at(COMPONENT_Y).VScale));
    
    // create the transformed coeffiecients:
    tCoeff_Y.resize(comp_height, vector<int> (comp_width, 0));
	// remove this
	cout << tCoeff_Y.size() << "----" << tCoeff_Y[0].size() << endl;

    m_YPicture_buffer_enc = new int*[upscale_height]; // height = rows
    for(int i = 0; i < upscale_height; ++i)
    {
        m_YPicture_buffer_enc[i] = new int[upscale_width]; // width = columns
    ...
    if(components.size() >= COMPONENT_Cb + 1) {
        ...
        comp_size = ceil((comp_width * comp_height)/
                             ((double)components.at(COMPONENT_Cb).HScale* (double) components.at(COMPONENT_Cb).VScale));
        
        // create the transformed coeffiecients:
//        tCoeff_Cb.resize(comp_height);
        tCoeff_Cb.resize(comp_height, vector<int>(comp_width, 0));
        
        m_CbPicture_buffer_enc = new int*[upscale_height]; // height = rows
        for(int i = 0; i < upscale_height; ++i)
        {
            m_CbPicture_buffer_enc[i] = new int[upscale_width]; // width = columns
    ...
    if(components.size() >= COMPONENT_Cr + 1) {
        ...
        comp_size = ceil((comp_width * comp_height)/
                             ((double)components.at(COMPONENT_Cr).HScale* (double) components.at(COMPONENT_Cr).VScale));
        
        // create the transformed coeffiecients:
//        tCoeff_Cr.resize(comp_size, vector<int> (64, 0));
         tCoeff_Cr.resize(comp_height, vector<int>(comp_width, 0));
        
        m_CrPicture_buffer_enc = new int*[upscale_height]; // height = rows
        for(int i = 0; i < upscale_height; ++i)
        {
            m_CrPicture_buffer_enc[i] = new int[upscale_width]; // width = columns
    ...
    // Create the luminance 2D buffer for the entire picture:
    m_YPicture_buffer = new uint_8*[jpegImageHeight]; // height = rows
    for(int i = 0; i <  jpegImageHeight; ++i)
    {
        m_YPicture_buffer[i] = new uint_8[jpegImageWidth]; // width = columns
    }
    
    // Create the Cb 2D buffer for the entire picture:
    m_CbPicture_buffer = new uint_8*[jpegImageHeight];
    for(int i = 0; i < jpegImageHeight; ++i)
    {
        m_CbPicture_buffer[i] = new uint_8[jpegImageWidth];
    }
    
    // Create the Cr 2D buffer for the entire picture:
    m_CrPicture_buffer = new uint_8*[jpegImageHeight];
    for(int i = 0; i < jpegImageHeight; ++i)
    {
        m_CrPicture_buffer[i] = new uint_8[jpegImageWidth];
    }
...
void jpeg_decoder::createRawImagePointers() {
    // Create the buffers for holding the Y'CbCr data
    for(int iComponent = 0; iComponent < components.size(); ++iComponent)
    {
        //int comp_size = ceil(jpegImageWidth*jpegImageHeight/(components.at(iComponent).HScale * components.at(iComponent).VScale));
		int comp_size = ceil(upscale_height*upscale_width / ((double)components.at(iComponent).HScale * (double) components.at(iComponent).VScale));
        rawImagePointers[iComponent] = new uint_8[comp_size];
    }
} // end createRawImagePointers
...
void jpeg_decoder::deleteRawImagePointers() {
    
    // Delete the buffers for holding the Y'CbCr data
    for(int iComponent = 0; iComponent < components.size(); ++iComponent)
    {
        if(rawImagePointers[iComponent] != nullptr)
        {
            delete [] rawImagePointers[iComponent];
            rawImagePointers[iComponent] = nullptr;
        }
    }
} // end deleteRawImagePointers
...
void jpeg_decoder::deleteRawPictureBufferPointersEnc() {
    
    int comp_height = ceil(upscale_height/(double) components.at(COMPONENT_Y).VScale);
    int comp_width = ceil(upscale_width/ (double) components.at(COMPONENT_Y).HScale);
    
    for(int i = 0; i < comp_height; ++i)
    {
        // delete each sub-array:
        if(m_YPicture_buffer_enc[i] != nullptr) {
            delete [] m_YPicture_buffer_enc[i];
            m_YPicture_buffer_enc[i] = nullptr;
        }
    }
    if(m_YPicture_buffer_enc != nullptr) {
        delete [] m_YPicture_buffer_enc;
        m_YPicture_buffer_enc = nullptr;
    }
    
    
    if(components.size() >= COMPONENT_Cb + 1) {
        comp_height = ceil(upscale_height / (double)components.at(COMPONENT_Cb).VScale);
        comp_width  = ceil(upscale_width / (double)components.at(COMPONENT_Cb).HScale);
        
        for(int i = 0; i < comp_height; ++i)
        {
            // delete each sub-array:
            if(m_CbPicture_buffer_enc[i] != nullptr) {
                delete [] m_CbPicture_buffer_enc[i];
                m_CbPicture_buffer_enc[i] = nullptr;
            }
            
        }
        if(m_CbPicture_buffer_enc != nullptr) {
            delete [] m_CbPicture_buffer_enc;
            m_CbPicture_buffer_enc = nullptr;
        }
        
    }
    
    if(components.size() >= COMPONENT_Cr + 1) {
		comp_height = ceil(upscale_height / (double)components.at(COMPONENT_Cr).VScale);
		comp_width = ceil(upscale_width / (double)components.at(COMPONENT_Cr).HScale);
        
        for(int i = 0; i < comp_height; ++i)
        {
            // delete each sub-array:
            if(m_CrPicture_buffer_enc[i] != nullptr) {
                delete [] m_CrPicture_buffer_enc[i];
                m_CrPicture_buffer_enc[i] = nullptr;
            }
            
        }
        if(m_CrPicture_buffer_enc != nullptr) {
            delete [] m_CrPicture_buffer_enc;
            m_CrPicture_buffer_enc = nullptr;
        }
        
    }
    
} // end deleteRawPictureBufferPointersEnc
...
void jpeg_decoder::Fankoosh(int dataBlock[8][8], int comp /* Current component */ ) {
    //    cout << "Adding block at " << X << "," << Y << " component " << comp <<endl;
    ...
    // These Y's should be scaled/repeated how much horizonatally and vertically
    int HScale = components[comp].HScale,   VScale = components[comp].VScale;
        ...
        for (int vfy = 0; vfy < VScale; ++vfy) {
            ...
            for (int x = 0; x < 8 ; ++x) {
                ...
                for (int i = 0; i < HScale; ++i) {
                    ...
                    // Set the pixel
                    //                    m_YPicture_buffer[realx][picture_y] = value;
                    m_YPicture_buffer[picture_y][realx] = value;
    ...
    //    if(mvp++ == 512)
    //    {
    //        std::string path_to_files = "/Users/Hossam_Amer/jpegCpp/TCM/TCM/";
    //        std::string pF = path_to_files + "Fankoosh.yuv";
    //        write_yuv_from_jpg_in_yuv(pF);
    //        exit(0);
    //    }
