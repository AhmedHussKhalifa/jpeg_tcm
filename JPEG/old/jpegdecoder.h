...
public:
    ...
    // Y buffer for the entire picture encoding:
    int** m_YPicture_buffer_enc;
    int** m_CbPicture_buffer_enc;
    int** m_CrPicture_buffer_enc;
...
public:
    ...
    unsigned char* rawImagePointers[ETF_FORMAT_MAX_COMPONENTS]; // TODO: to be removed
    ...
    // read Bits in a queue format
    uint_16 readBits(int len); // not used
    
    // Loop steps
    void readAppSegment(unsigned char marker); // TODO: incomplete
    void readMetaDataJFIF(); // TODO: incomplete
    void readMetaDataJFXX(int headerLength); // TODO: incomplete
    ...
    uint_16 readImageData(); // not used
    
    // HuffmanTable huffmanTable;
    void readHuffmanBlock(int* block, int currentComponent); // not used
    bool readMoreData(unsigned int &data, unsigned int &currentDataLength); // not used
    ...
    void deleteRawPictureBufferPointersEnc();
    ...
    void addBlock(int dataBlock[8][8], int currentComponent = COMPONENT_Y); // unused (TODO: needs more work)
    ...
    // unused function - was used for testing: -- should be removed later
    void Fankoosh(int dataBlock[8][8], int currentComponent = COMPONENT_Y);
    
    // test IDCT -> DCT
    void Fankoosh(int tCoeff [8][8], int out_block8x8[8][8], int in_block8x8[8][8], int currentComponent);
    void perform_fdct(int outBlock [8][8], int inBlock[8][8]);
    int func_dct(int x, int y, const int block[8][8]);
    float C_dct(int u);
    ...
    void ycrcb_to_rgb24_block8x8(int w, int h, int imgx, int imgy, int imgw, int imgh);
    ...
    // transpose for Y component of an image
    void transposeImage();
    ...
    void DequantizeBlock(int block[64], int currentComponent); // not used
    
    
    void IDCT (int block8x8[8][8], int block2[8][8]);
    void multiplyWithQuantizationTable(int dataBlock[8][8], int currentComponent);
    void addBlockOld(int dataBlock[8][8], int currentComponent);
    void addBlockSubsamplingOld(int dataBlock[8][8], int currentComponent);
    ...
    inline void printNBits(int nbits_wanted)
    {
        ...
        cout << "Read bits: " << std::hex << result  << endl;
    }
